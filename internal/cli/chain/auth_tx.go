...
func sign(cctx client.Context, txBuilder client.TxBuilder, txFactory tx.Factory, from string) error {
	_, fromName, _, err := client.GetFromFields(cctx, txFactory.Keybase(), from)
	if err != nil {
		return fmt.Errorf("error getting account from keybase: %w", err)
	}

	if err = SignTx(txFactory, cctx, fromName, txBuilder, true, true); err != nil {
		return err
	}
...
func multisigSign(cctx client.Context, txBuilder client.TxBuilder, txFactory tx.Factory, multisig string) error {
	multisigAddr, multisigName, _, err := client.GetFromFields(cctx, txFactory.Keybase(), multisig)
	if err != nil {
		return fmt.Errorf("error getting account from keybase: %w", err)
	}

	fromRecord, err := cctx.Keyring.Key(cctx.FromName)
	if err != nil {
		return fmt.Errorf("error getting account from keybase: %w", err)
	}
	...
	isSigner, err := isMultisigSigner(cctx, multisigPubKey, fromPubKey)
	if err != nil {
		return err
	}
...
func signTx(cmd *cobra.Command, cctx client.Context, txF tx.Factory, newTx sdk.Tx) error {
	...
	if multisig != "" {
		...
		err = authclient.SignTx(txF, cctx, cctx.FromName, txBuilder, cctx.Offline, overwrite)
	}
	if err != nil {
		return err
	}
...
func readTxAndInitContexts(cctx client.Context, cmd *cobra.Command, filename string) (client.Context, tx.Factory, sdk.Tx, error) {
	stdTx, err := authclient.ReadTxFromFile(cctx, filename)
	if err != nil {
		return cctx, tx.Factory{}, nil, err
	}
	...
	return cctx, txFactory, stdTx, nil
}
