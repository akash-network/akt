...
and export your keys in ASCII-armored encrypted format.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			buf := bufio.NewReader(cctx.Input)
			unarmored, _ := cmd.Flags().GetBool(flagUnarmoredHex)
			unsafe, _ := cmd.Flags().GetBool(flagUnsafe)

			if unarmored && unsafe {
				return exportUnsafeUnarmored(cmd, args[0], buf, cctx.Keyring)
			...
			armored, err := cctx.Keyring.ExportPrivKeyArmor(args[0], encryptPassword)
			if err != nil {
				return err
			}
