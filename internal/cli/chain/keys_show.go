...
func runShowCmd(cmd *cobra.Command, args []string) (err error) {
	...
	outputFormat := cctx.OutputFormat

	if len(args) == 1 {
		k, err = fetchKey(cctx.Keyring, args[0])
		if err != nil {
		...
		for i, keyRef := range args {
			...
			k, err := fetchKey(cctx.Keyring, keyRef)
			if err != nil {
	...
	if isOutputSet {
		cctx.OutputFormat, _ = cmd.Flags().GetString(flags.FlagOutput)
	}
