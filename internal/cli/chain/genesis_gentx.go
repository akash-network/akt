...
func readUnsignedGenTxFile(cctx client.Context, r io.Reader) (sdk.Tx, error) {
	bz, err := io.ReadAll(r)
	if err != nil {
		return nil, err
	}

	aTx, err := cctx.TxConfig.TxJSONDecoder()(bz)
	if err != nil {
		return nil, err
	}
...
func writeSignedGenTx(cctx client.Context, outputDocument string, tx sdk.Tx) error {
	...
	txj, err := cctx.TxConfig.TxJSONEncoder()(tx)
	if err != nil {
		return err
	}
