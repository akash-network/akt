...
func runListCmd(cmd *cobra.Command, _ []string) error {
	...
	records, err := cctx.Keyring.List()
	if err != nil {
		return err
	}

	if len(records) == 0 && cctx.OutputFormat == cflags.OutputFormatText {
		cmd.Println("No records were found in keyring")
		return nil
	}

	if ok, _ := cmd.Flags().GetBool(flagListNames); !ok {
		return printKeyringRecords(cmd.OutOrStdout(), records, cctx.OutputFormat)
	}
...
func ListKeyTypesCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			keyring, _ := cctx.Keyring.SupportedAlgorithms()
			cmd.Printf("%+q\n", keyring)
			return nil
		},
	}
