...
func ShowValidatorCmd() *cobra.Command {
	cmd := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			bz, err := cctx.Codec.MarshalInterfaceJSON(sdkPK)
			if err != nil {
				return err
			}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			blocks, err := rpc.QueryBlocks(cctx, page, limit, query, orderBy)
			if err != nil {
				return err
			}

			return cctx.PrintProto(blocks)
		},
	}
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case cflags.TypeHeight:
				...
				output, err := rpc.GetBlockByHeight(cctx, &height)
				if err != nil {
					return err
				}
			...
			case cflags.TypeHash:
				...
				output, err := rpc.GetBlockByHash(cctx, args[0])
				if err != nil {
					return err
				}
...
func QueryBlockResultsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// so we can't print it using cctx.PrintProto
			...
			return cctx.PrintRaw(blockResStr)
		},
	}
