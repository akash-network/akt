...
func RenderOraclePrices(res *types.QueryPricesResponse) string {
	...
	for _, p := range res.Prices {
		price := FormatPriceDec(p.State.Price)
...
func RenderAggregatedPrice(res *types.QueryAggregatedPriceResponse) string {
	...
	KV(&buf, "TWAP", Bold(FormatPriceDec(ap.TWAP)))
	KV(&buf, "Median", FormatPriceDec(ap.MedianPrice))
	KV(&buf, "Min", FormatPriceDec(ap.MinPrice))
	KV(&buf, "Max", FormatPriceDec(ap.MaxPrice))
