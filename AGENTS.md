...
- **Build**: `GOWORK=off make akt` — the compiled binary is placed in `.cache/bin/`. The Makefile defaults the direnv-managed cache variables to the same `.cache` layout when they are unset, so this command also works in a bare checkout.
